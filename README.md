...
```bash
SQUAREFLO_API_URL=https://test-site-rinae.flodev.ca/api/v1
...
```bash
VITE_SQUAREFLO_API_URL=https://test-site-rinae.flodev.ca/api/v1
...
The app falls back to polished agency content when no CMS key is configured, then uses Squareflo pages, navigation, settings, FAQs, reviews, and forms when credentials are present.
